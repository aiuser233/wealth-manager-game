...
- `QuestEngine` 构造时注入全量任务与人生线，内部按日期排序。
- `checkQuests(currentDate, dateShift?)`：每次帧推进后调用，返回**最早一个到期未完成任务**；`requires` 字段构成跨卷前置链（如 q2_01 需要 q1_12 完成），到期但前置未完成则不触发——**34 条任务已挂链**，`content:check` 的 ref-check 校验 requires 必须指向真实任务 id；`dateShift` 是二周目的任务日期漂移表（`{questId: 新日期}`），初见局不传。
...
**内容量**（第二轮深化批后）：题库 2689 / 词条 195 / 主线 68 章（五卷各 12 + 卷六彩蛋 8）/ 人生线 9 条 77 节点 / 复盘卡 30 / 导演事件 105 / 产品货架 47 / 客户 30 / 随机事件 80 / 行动小剧场 60 场景（轮值 14 + 售后 9 + 社交 9 + 外拓 10 + 复盘 18，另轮值每题双选）/ 接待需求 18 种 / 考试科目 8；双审元数据 2864/2884 条（status=draft 待人工审校——**发布培训包前必须人工审完翻 approved**，这是当前唯一硬性欠账，见规划合并文档 §15）。
