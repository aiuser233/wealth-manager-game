...
| `TopBar.vue` | 页签导航+日期+职级显示；工作台 tab 红点=**未读新闻数**（`state.news.length - newsSeen`，进入行情终端即已读清零，hover 提示条数） |
