...
- `QuestEngine` 构造时注入全量任务与人生线，内部按日期排序。
- `checkQuests(currentDate, dateShift?)`：每次帧推进后调用，返回**最早一个到期未完成任务**；`requires` 字段构成跨卷前置链（如 q2_01 需要 q1_12 完成），到期但前置未完成则不触发——**38 条任务已挂链**，`content:check` 的 ref-check 校验 requires 必须指向真实任务 id；`dateShift` 是二周目的任务日期漂移表（`{questId: 新日期}`），初见局不传。
...
| `events.ts` | **105 导演事件**（81 基础 + 深化批 12 条：基金限购潮/108 基点降息/股指期货上市/温州民间借贷/国债期货重启/一带一路/811 余波/熔断后冷静期/民企纾困/LPR 二次下调/个人养老金实施/新国九条 + 第二轮 12 条：2009 十大产业振兴/2011 理财乱象/2012 金改/2017 去杠杆/2020 注册制创业板/2020 蚂蚁暂停/2021 双减/2023 地产白名单/2023 特别国债/2024 互换便利/2025 化债二阶段/2025 机器人行情——小冲击年代纹理，不动锚点），每条：确切日期/type（policy/director/black_swan）/news 文案/shocks 因子冲击/duration_days 持续/sentiment 情绪/force_day 强制降帧标记/unlock_knowledge | 约 28 条 force_day/black_swan 进讲师大屏与金手指记忆 |
...
| 文件 | 内容 | 说明 |
|---|---|---|
| `clients.ts`+`clients-deep.ts` | **30 核心客户**（10 原始 + 剧情位 + 深化 12 位 + 第二轮深化 8 位（医生/主播/MCN/县城个体/科技高管/非遗/丁克工程师/农场主）：个体户/外贸企业主/拆迁户/海归/县城教师/货车司机/网贷踩坑店主/前基金经理/直播创业者/出口商/返聘工程师/留学生家庭——覆盖年代联动客群） | `ClientDef` 全 KYC：tier 档位、risk R1-R5、finance 六项资产负债、behaviors 行为标签（yield_chasing 追高/risk_averse…）、trust 信任 0-100 |
| `products.ts` | **47 产品**跨 20 年货架（2006 凭证式国债 → 2013 宝宝类 → 2015 分级基金 → 2018 净值型理财 → 2021 雪球/REITs → 2023 红利低波 → 2024 个人养老金；第二轮 +15：通知存款/智能存款靠档/现金管理池/预期收益型/每日理财/固收增强3年/300ETF联接/小盘增强/红利低波/REITs/投连险/百万医疗/积存金/中性策略私募/碳主题理财） | `ProductDef`：risk_level/min_amount/term_days/issuer（架空）/benchmark_pa/**era 上架区间**/nav_model（净值型因子 loading）或 fixed_rate_pa（存款） |

### 3.3 剧情内容

| 文件 | 内容 | 说明 |
|---|---|---|
| `quests.ts`~`quests6.ts` | **五卷主线各 12 任务 + 卷六彩蛋 8 章**（共 68），日期驱动：卷一 2006-2009（疯牛→5·30→6124→1664→4万亿）、卷二 2010-2015（负利率→钱荒→杠杆牛熊）、卷三 2016-2020（熔断→贸易战→资管新规→疫情）、卷四 2021-2023（抱团瓦解→破净潮→AI 元年→第一个下属）、卷五 2024-2025（微盘股危机→9·24→个人养老金→AI 换脸防诈→传承→卷终）、卷六彩蛋（2025 年 8 章回访式小传：拆迁款/旧保单检视/养老金领取/传承盘点/前基金经理复盘/主播现金流/非遗授权/万家灯火终章） | `QuestItem`：dialogues 多角色对话演出（speaker/text/mood）、choices 三选一带 grade 与 effects（trust/pro/comm/sales/rep/stress/aum/unlockKnowledge）、teach 教学 tag；每卷末任务 `volN_end` 触发三维终评，卷五终章触发六结局判定 |
| `lifelines.ts` | **9 条人生线 77 节点**（第七条陈曼师父线 9：2006 教案→2008 婚礼受益人→2010 教育金→2013 拒跳槽→2015 股灾零伤→2019 升副总→2021 自曝反例→2023 退休返聘→2025 师徒账本；第八条唐薇线 8：2013 宝宝类启蒙→2015 牛市与股灾→2018 定投重启→2021 抱团归因→2022 替母问破净（转介绍）→2024 婚前账本→2025 成为传播节点） |：王秀兰 9（养老客群）、李建国 9（工薪客群）、周宏图 9（企业主）、吴建国 8（大众客群）、何志敏 9（年轻客群→合规成长）、周远航 8（00 后徒弟线：少年目击杠杆→模拟盘→新人冲单→9·24 冲动→AI 质询→拦住自己→出师，与团队系统 sub_zhouyh 同人同弧） | `LifeLineDef`：year/month/client/title/text/trustReq 信任门槛/effects；信任达标自动触发演出 |
| `random-events.ts` | **80 随机事件**按年代池化（36 基础 + 深化批 randomEventsDeep 24 + 第二轮 randomEventsDeep2 20：开户送礼/净值挤兑/银保暗访/信用卡分期/宝类冲击/钱荒恐慌/沪港通/打新归集/民间借贷失联/资管新规解读会/P2P 求助/房价辩论/科创板适当性/疫情远程/抱团安抚/雪球咨询/养老金冲量/AI 投诉/跨境尽调/遗产继承/降息挪储/数据泄露/购金热/离职创业） | 分支三档 risk（稳妥/灰色/红线），红线累计违规 |
| `knowledge.ts`~`knowledge8.ts` | **195 知识词条**八批：基础/产品/市场/规划/合规/行为金融六类（第七批 18 条补 2020s：跨境理财通/个人养老金细节/REITs/雪球/ETF 时代/利率市场化/市值管理/耐心资本/金监总局/数据要素/适老金融/绿色金融/数字人民币/资产服务信托等） | 五件套强制：what 是什么/why 为什么/how 怎么用/pitfall 常见坑/quiz 考一考 + unlockYear 解锁年 + tags 题库映射 + review 双审元数据 |
...
| `DossierPanel.vue` | 生涯档案页（C1.1）：careerLog 按卷号分六卷时间线（gradeLabel 徽章：best ✦/good ✓/normal ○/bad ✗）、八维生涯卡片（职级/AUM/主线 x/68/人生线节点/信任均值/违规/最佳抉择率/证书）、Avatar+成就 chip、**HTML 导出（自包含 CSS，打印即 PDF）** |
...
60 个主线任务中 24 个挂 `requires` 字段，构成跨卷叙事链（q1_04→…→q1_12→q2_01→…→q5_12）：后置任务即使日期到期，若前置未完成也不触发（玩家补做前置后下次帧推进即触发）。链路覆盖：卷一 8 条承接链、卷二 5 条、卷三 4 条、卷四 3 条、卷五 3 条 + 卷间衔接 4 条。`ref-check` 校验 requires 必须指向真实任务 id，防手滑断链。
...
**内容量**（第三轮深化批后）：题库 3489 / 词条 226 / 主线 74 章（五卷各 12 + 卷六彩蛋 14）/ 人生线 11 条 93 节点 / 复盘卡 35 / 导演事件 120 / 产品货架 53 / 客户 35 / 随机事件 95 / 行动小剧场 60 场景 / 接待需求 21 种 / 考试科目 8；**题-词缝合：exams28-35 八批 800 题 knowledge_tags 100% 命中词条体系（TAG_SEAL 机器闸）**；双审元数据 3695/3715 条（status=draft 待人工审校——**发布培训包前必须人工审完翻 approved**，这是当前唯一硬性欠账，见规划合并文档 §15）。
