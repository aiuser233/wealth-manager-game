...
- **行动系统 `doAction(type)`**：8 种行动（reception 接待/lobby 厅堂/outreach 外拓/study 学习/review 复盘/aftersale 售后/social 同事/rest 休息），每种有 AP 消耗（共用帧上限）、精力消耗（帧越粗单次越耗）与属性增益/压力变化。接待走独立的对话玩法（见 §5.2）。NEED_POOL 14 种需求场景（深化批 +6：biz_split 企业流动资金/study_abroad 留学购汇/house_wealth 房产杠杆/wedding_fund 婚嫁金/pension_topup 养老补充/payroll_biz 代发切入）。
...
- `QuestEngine` 构造时注入全量任务与人生线，内部按日期排序。
- `checkQuests(currentDate, dateShift?)`：每次帧推进后调用，返回**最早一个到期未完成任务**；`requires` 字段构成跨卷前置链（如 q2_01 需要 q1_12 完成），到期但前置未完成则不触发——**24 条任务已挂链**，`content:check` 的 ref-check 校验 requires 必须指向真实任务 id；`dateShift` 是二周目的任务日期漂移表（`{questId: 新日期}`），初见局不传。
...
| `events.ts` | **93 导演事件**（81 基础 + 深化批 12 条微事件：基金限购潮/108 基点降息/股指期货上市/温州民间借贷/国债期货重启/一带一路/811 余波/熔断后冷静期/民企纾困/LPR 二次下调/个人养老金实施/新国九条——小冲击年代纹理，不动锚点），每条：确切日期/type（policy/director/black_swan）/news 文案/shocks 因子冲击/duration_days 持续/sentiment 情绪/force_day 强制降帧标记/unlock_knowledge | 约 28 条 force_day/black_swan 进讲师大屏与金手指记忆 |
...
| 文件 | 内容 | 说明 |
|---|---|---|
| `clients.ts`+`clients-deep.ts` | **22 核心客户**（10 原始 + 剧情位 + 深化 12 位：个体户/外贸企业主/拆迁户/海归/县城教师/货车司机/网贷踩坑店主/前基金经理/直播创业者/出口商/返聘工程师/留学生家庭——覆盖年代联动客群） | `ClientDef` 全 KYC：tier 档位、risk R1-R5、finance 六项资产负债、behaviors 行为标签（yield_chasing 追高/risk_averse…）、trust 信任 0-100 |
...
| `lifelines.ts` | **8 条人生线 69 节点**（第七条陈曼师父线 9：2006 教案→2008 婚礼受益人→2010 教育金→2013 拒跳槽→2015 股灾零伤→2019 升副总→2021 自曝反例→2023 退休返聘→2025 师徒账本；第八条唐薇线 8：2013 宝宝类启蒙→2015 牛市与股灾→2018 定投重启→2021 抱团归因→2022 替母问破净（转介绍）→2024 婚前账本→2025 成为传播节点） |：王秀兰 9（养老客群）、李建国 9（工薪客群）、周宏图 9（企业主）、吴建国 8（大众客群）、何志敏 9（年轻客群→合规成长）、周远航 8（00 后徒弟线：少年目击杠杆→模拟盘→新人冲单→9·24 冲动→AI 质询→拦住自己→出师，与团队系统 sub_zhouyh 同人同弧） | `LifeLineDef`：year/month/client/title/text/trustReq 信任门槛/effects；信任达标自动触发演出 |
| `random-events.ts` | **60 随机事件**按年代池化（36 基础 + 深化批 randomEventsDeep 24：开户送礼/净值挤兑/银保暗访/信用卡分期/宝类冲击/钱荒恐慌/沪港通/打新归集/民间借贷失联/资管新规解读会/P2P 求助/房价辩论/科创板适当性/疫情远程/抱团安抚/雪球咨询/养老金冲量/AI 投诉/跨境尽调/遗产继承/降息挪储/数据泄露/购金热/离职创业） | 分支三档 risk（稳妥/灰色/红线），红线累计违规 |
| `knowledge.ts`~`knowledge7.ts` | **165 知识词条**七批：基础/产品/市场/规划/合规/行为金融六类（第七批 18 条补 2020s：跨境理财通/个人养老金细节/REITs/雪球/ETF 时代/利率市场化/市值管理/耐心资本/金监总局/数据要素/适老金融/绿色金融/数字人民币/资产服务信托等） | 五件套强制：what 是什么/why 为什么/how 怎么用/pitfall 常见坑/quiz 考一考 + unlockYear 解锁年 + tags 题库映射 + review 双审元数据 |
...
| `DossierPanel.vue` | 生涯档案页（C1.1）：careerLog 按卷号分五卷时间线（gradeLabel 徽章：best ✦/good ✓/normal ○/bad ✗）、八维生涯卡片（职级/AUM/主线 x/60/人生线节点/信任均值/违规/最佳抉择率/证书）、Avatar+成就 chip、**HTML 导出（自包含 CSS，打印即 PDF）** |
...
**内容量**：题库 2089 / 词条 165 / 任务 60 / 人生线 69 节点 / 复盘卡 24 / 导演事件 93 / 产品 32 / 客户 22 / 随机事件 60 / 行动小剧场 41 场景 / 接待需求 14 种 / 考试科目 8；双审元数据 2234/2254 条（status=draft 待人工审校——**发布培训包前必须人工审完翻 approved**，这是当前唯一硬性欠账，见规划合并文档 §15）。
